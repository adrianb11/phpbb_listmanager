...
class card_controller
{
	 ...
	 */
	public function __construct(\phpbb\controller\helper $helper, \phpbb\template\template $template, $root_path, $php_ext, \phpbb\user $user, \phpbb\auth\auth $auth, \phpbb\request\request $request, \phpbb\content_visibility $content_visibility, \phpbb\db\driver\driver_interface $db, \phpbb\config\config $config, \phpbb\cache\service $cache)
	{
		$this->helper				= $helper;
	...
	function handle()
	{
		...
		{
			$this->web_root_path = append_sid($this->root_path);
			...
			return $this->helper->render('@adrianb11_listmanager/listmanager_topic_display.html');
		}
	}
	...
	/**
	 * Get topic data.  Modified copy of viewtopic.php
	 */
	function get_topic()
	{
		// Initial var setup
		...
		*/
		...
		if (!$topic_data)
		{
			...
			if ($post_id && $topic_id)
			{
				redirect(append_sid("{$this->root_path}viewtopic.$this->php_ext", "t=$topic_id" . (($forum_id) ? "&amp;f=$forum_id" : ''), true, false, true));
			}
		...
		$this->user->page['forum'] = $forum_id;

		// Now we know the forum_id and can check the permissions
		if (!$this->content_visibility->is_visible('topic', $forum_id, $topic_data))
		{
			trigger_error('NO_TOPIC');
		}
		...
		if ($view == 'print' && !$this->auth->acl_get('f_print', $forum_id))
		{
		...
		$viewtopic_url = append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : ''), true, false, true);
		...
		//	'key'			=> array('LANG_KEY', $this->userHasPermissions),
		...
		$this->template->assign_vars(array(
			...
			'TOPIC_AUTHOR_FULL'		=> str_replace($this->web_root_path, $this->root_path, get_username_string('full', $topic_data['topic_poster'], $topic_data['topic_first_poster_name'], $topic_data['topic_first_poster_colour'])),
			'TOPIC_AUTHOR_COLOUR'	=> str_replace($this->web_root_path, $this->root_path, get_username_string('colour', $topic_data['topic_poster'], $topic_data['topic_first_poster_name'], $topic_data['topic_first_poster_colour'])),
			'TOPIC_AUTHOR'			=> str_replace($this->web_root_path, $this->root_path, get_username_string('username', $topic_data['topic_poster'], $topic_data['topic_first_poster_name'], $topic_data['topic_first_poster_colour'])),

			'TOTAL_POSTS'	=> $this->user->lang('VIEW_TOPIC_POSTS', (int) $total_posts),
			'U_MCP' 		=> ($this->auth->acl_get('m_', $forum_id)) ? append_sid("{$this->root_path}mcp.$this->php_ext", "i=main&amp;mode=topic_view&amp;f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : ''), true, $this->user->session_id, true) : '',
			...
			'S_TOPIC_ACTION' 		=> append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"), true, false, true),
			...
			'S_SEARCHBOX_ACTION'	=> append_sid("{$this->root_path}search.$this->php_ext", false, true, false, true),
			...
			'U_FORUM'				=> $server_path,
			'U_VIEW_TOPIC' 			=> append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . (strlen($u_sort_param) ? "&amp;$u_sort_param" : ''), true, false, true),
			'U_CANONICAL'			=> generate_board_url() . '/' . append_sid("viewtopic.$this->php_ext", "t=$topic_id" . (($start) ? "&amp;start=$start" : ''), true, '', false, true),
			'U_VIEW_FORUM' 			=> append_sid("{$this->root_path}viewforum.$this->php_ext", 'f=' . $forum_id, true, false, true),
			'U_VIEW_OLDER_TOPIC'	=> append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id&amp;view=previous", true, false, true),
			'U_VIEW_NEWER_TOPIC'	=> append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id&amp;view=next", true, false, true),
			'U_PRINT_TOPIC'			=> ($this->auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . '&amp;view=print' : '',
			'U_EMAIL_TOPIC'			=> ($this->auth->acl_get('f_email', $forum_id) && $this->config['email_enable']) ? append_sid("{$this->root_path}memberlist.$this->php_ext", "mode=email&amp;t=$topic_id", true, false, true) : '',

			'U_WATCH_TOPIC'			=> str_replace($this->web_root_path, $this->root_path, $s_watching_topic['link']),
			'U_WATCH_TOPIC_TOGGLE'	=> str_replace($this->web_root_path, $this->root_path, $s_watching_topic['link_toggle']),
			...
			'U_POST_NEW_TOPIC' 		=> ($this->auth->acl_get('f_post', $forum_id) || $this->user->data['user_id'] == ANONYMOUS) ? append_sid("{$this->root_path}posting.$this->php_ext", "mode=post&amp;f=$forum_id", true, false, true) : '',
			'U_POST_REPLY_TOPIC' 	=> ($this->auth->acl_get('f_reply', $forum_id) || $this->user->data['user_id'] == ANONYMOUS) ? append_sid("{$this->root_path}posting.$this->php_ext", "mode=reply&amp;f=$forum_id&amp;t=$topic_id", true, false, true) : '',
			'U_BUMP_TOPIC'			=> (bump_topic_allowed($forum_id, $topic_data['topic_bumped'], $topic_data['topic_last_post_time'], $topic_data['topic_poster'], $topic_data['topic_last_poster_id'])) ? append_sid("{$this->root_path}posting.$this->php_ext", "mode=bump&amp;f=$forum_id&amp;t=$topic_id&amp;hash=" . generate_link_hash("topic_$topic_id"), true, false, true) : '')
		);
		...
		if (!empty($topic_data['poll_start']))
		{
			...
			if ($update && $s_can_vote)
			{
				...
				{
					$redirect_url = append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"), true, false, true);
				...
				if ($this->request->is_ajax())
				{
					...
					$data = array(
						...
						'can_vote'			=> !count($valid_user_votes) || ($this->auth->acl_get('f_votechg', $forum_id) && $topic_data['poll_vote_change']),
					);
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			if (!isset($user_cache[$poster_id]))
			{
				if ($poster_id == ANONYMOUS)
				{
					...
					$this->user_rank_data = phpbb_get_user_rank($row, false);
					$user_cache[$poster_id]['rank_title'] = $this->user_rank_data['title'];
					$user_cache[$poster_id]['rank_image'] = $this->user_rank_data['img'];
					$user_cache[$poster_id]['rank_image_src'] = $this->user_rank_data['img_src'];
				}
				else
				{
					$this->user_sig = '';
					...
					$user_cache_data = array(
						...
						'username'			=> $row['username'],
						'user_colour'		=> $row['user_colour'],
						'contact_user' 		=> $this->user->lang('CONTACT_USER', str_replace($this->web_root_path, $this->root_path, get_username_string('username', $poster_id, $row['username'], $row['user_colour'], $row['username']))),

						'online'		=> false,
						'jabber'		=> ($this->config['jab_enable'] && $row['user_jabber'] && $this->auth->acl_get('u_sendim')) ? append_sid("{$this->root_path}memberlist.$this->php_ext", "mode=contact&amp;action=jabber&amp;u=$poster_id", true, false, true) : '',
						'search'		=> ($this->config['load_search'] && $this->auth->acl_get('u_search')) ? append_sid("{$this->root_path}search.$this->php_ext", "author_id=$poster_id&amp;sr=posts", true, false, true) : '',

						'author_full'		=> str_replace($this->web_root_path, $this->root_path, get_username_string('full', $poster_id, $row['username'], $row['user_colour'])),
						'author_colour'		=> str_replace($this->web_root_path, $this->root_path, get_username_string('colour', $poster_id, $row['username'], $row['user_colour'])),
						'author_username'	=> str_replace($this->web_root_path, $this->root_path, get_username_string('username', $poster_id, $row['username'], $row['user_colour'])),
						'author_profile'	=> str_replace($this->web_root_path, $this->root_path, get_username_string('profile', $poster_id, $row['username'], $row['user_colour'])),
					);

					$user_cache[$poster_id] = $user_cache_data;

					$this->user_rank_data = phpbb_get_user_rank($row, $row['user_posts']);
					$user_cache[$poster_id]['rank_title'] = $this->user_rank_data['title'];
					$user_cache[$poster_id]['rank_image'] = $this->user_rank_data['img'];
					$user_cache[$poster_id]['rank_image_src'] = $this->user_rank_data['img_src'];
					...
					{
						$user_cache[$poster_id]['email'] = ($this->config['board_email_form'] && $this->config['email_enable']) ? append_sid("{$this->root_path}memberlist.$this->php_ext", "mode=email&amp;u=$poster_id", true, false, true) : (($this->config['board_hide_emails'] && !$this->auth->acl_get('a_email')) ? '' : 'mailto:' . $row['user_email']);
					}
					else
					{
		...
		for ($i = 0, $end = count($post_list); $i < $end; ++$i)
		{
			...
			if (($row['post_edit_count'] && $this->config['display_last_edited']) || $row['post_edit_reason'])
			{
				...
				if (!count($post_edit_list) && ($row['post_edit_reason'] || ($row['post_edit_user'] && !isset($user_cache[$row['post_edit_user']]))))
				{
					...
					while ($this->user_edit_row = $this->db->sql_fetchrow($result2))
					{
						$post_edit_list[$this->user_edit_row['user_id']] = $this->user_edit_row;
					}
				...
				if ($row['post_edit_reason'])
				{
					...
					{
						$display_username = str_replace($this->web_root_path, $this->root_path, get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']));
					}
					else
					{
						$display_username = str_replace($this->web_root_path, $this->root_path, get_username_string('full', $row['post_edit_user'], $post_edit_list[$row['post_edit_user']]['username'], $post_edit_list[$row['post_edit_user']]['user_colour']));
					}
				...
				else
				{
					...
					{
						$display_username = str_replace($this->web_root_path, $this->root_path, get_username_string('full', $row['post_edit_user'], $user_cache[$row['post_edit_user']]['username'], $user_cache[$row['post_edit_user']]['user_colour']));
					}
			...
			if ($row['post_visibility'] == ITEM_DELETED && $row['post_delete_user'])
			{
				...
				if (!count($post_delete_list) && ($row['post_delete_reason'] || ($row['post_delete_user'] && !isset($user_cache[$row['post_delete_user']]))))
				{
					...
					$result2 = $this->db->sql_query($sql);
					while ($this->user_delete_row = $this->db->sql_fetchrow($result2))
					{
						$post_delete_list[$this->user_delete_row['user_id']] = $this->user_delete_row;
					}
					$this->db->sql_freeresult($result2);
				...
				$display_postername = str_replace($this->web_root_path, $this->root_path, get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']));
				...
				{
					$display_username = str_replace($this->web_root_path, $this->root_path, get_username_string('full', $row['post_delete_user'], $user_cache[$row['post_delete_user']]['username'], $user_cache[$row['post_delete_user']]['user_colour']));
				}
			...
			$s_cannot_edit_locked = ($topic_data['topic_status'] == ITEM_LOCKED && !$this->auth->acl_get('m_lock', $forum_id)) || $row['post_edit_locked'];
			...
			$softdelete_allowed = $force_softdelete_allowed || (($this->auth->acl_get('m_softdelete', $forum_id) ||
				($this->auth->acl_get('f_softdelete', $forum_id) && $this->user->data['user_id'] == $poster_id)) && ($row['post_visibility'] != ITEM_DELETED));

			$permanent_delete_allowed = $force_delete_allowed || ($this->auth->acl_get('m_delete', $forum_id) ||
			...
			{
				$u_pm = append_sid("{$this->root_path}ucp.$this->php_ext", 'i=pm&amp;mode=compose&amp;action=quotepost&amp;p=' . $row['post_id'], true, false, true);
			}

			//
			$post_row = array(
				'POST_AUTHOR_FULL'		=> ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_full'] : str_replace($this->web_root_path, $this->root_path, get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])),
				'POST_AUTHOR_COLOUR'	=> ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_colour'] : str_replace($this->web_root_path, $this->root_path, get_username_string('colour', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])),
				'POST_AUTHOR'			=> ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_username'] : str_replace($this->web_root_path, $this->root_path, get_username_string('username', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])),
				'U_POST_AUTHOR'			=> ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_profile'] : str_replace($this->web_root_path, $this->root_path, get_username_string('profile', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])),
				...
				'POST_SUBJECT'		=> $row['post_subject'],
				'MESSAGE'			=> str_replace($this->web_root_path, $this->root_path, $message),
				...
				'U_EDIT'			=> ($edit_allowed) ? append_sid("{$this->root_path}posting.$this->php_ext", "mode=edit&amp;f=$forum_id&amp;p={$row['post_id']}", true, false, true) : '',
				'U_QUOTE'			=> ($quote_allowed) ? append_sid("{$this->root_path}posting.$this->php_ext", "mode=quote&amp;f=$forum_id&amp;p={$row['post_id']}", true, false, true) : '',
				'U_INFO'			=> ($this->auth->acl_get('m_info', $forum_id)) ? append_sid("{$this->root_path}mcp.$this->php_ext", "i=main&amp;mode=post_details&amp;f=$forum_id&amp;p=" . $row['post_id'], true, $this->user->session_id, true) : '',
				'U_DELETE'			=> ($delete_allowed) ? append_sid("{$this->root_path}posting.$this->php_ext", 'mode=' . (($softdelete_allowed) ? 'soft_delete' : 'delete') . "&amp;f=$forum_id&amp;p={$row['post_id']}", true, false, true) : '',
				...
				'U_APPROVE_ACTION'		=> append_sid("{$this->root_path}mcp.$this->php_ext", "i=queue&amp;p={$row['post_id']}&amp;f=$forum_id&amp;redirect=" . urlencode(str_replace('&amp;', '&', $viewtopic_url . '&amp;p=' . $row['post_id'] . '#p' . $row['post_id'])), true, false, true),
				'U_REPORT'			=> ($this->auth->acl_get('f_report', $forum_id)) ? $this->helper->route('phpbb_report_post_controller', array('id' => $row['post_id'])) : '',
				'U_MCP_REPORT'		=> ($this->auth->acl_get('m_report', $forum_id)) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=reports&amp;mode=report_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $this->user->session_id, true) : '',
				'U_MCP_APPROVE'		=> ($this->auth->acl_get('m_approve', $forum_id)) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=queue&amp;mode=approve_details&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $this->user->session_id, true) : '',
				'U_MCP_RESTORE'		=> ($this->auth->acl_get('m_approve', $forum_id)) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=queue&amp;mode=' . (($topic_data['topic_visibility'] != ITEM_DELETED) ? 'deleted_posts' : 'deleted_topics') . '&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $this->user->session_id, true) : '',
				'U_MINI_POST'		=> append_sid("{$this->root_path}viewtopic.$this->php_ext", 'p=' . $row['post_id'], true, false, true) . '#p' . $row['post_id'],
				'U_NEXT_POST_ID'	=> ($i < $i_total && isset($rowset[$post_list[$i + 1]])) ? $rowset[$post_list[$i + 1]]['post_id'] : '',
				'U_PREV_POST_ID'	=> $prev_post_id,
				'U_NOTES'			=> ($this->auth->acl_getf_global('m_')) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=notes&amp;mode=user_notes&amp;u=' . $poster_id, true, $this->user->session_id, true) : '',
				'U_WARN'			=> ($this->auth->acl_get('m_warn') && $poster_id != $this->user->data['user_id'] && $poster_id != ANONYMOUS) ? append_sid("{$this->root_path}mcp.$this->php_ext", 'i=warn&amp;mode=warn_post&amp;f=' . $forum_id . '&amp;p=' . $row['post_id'], true, $this->user->session_id, true) : '',
				...
				'L_IGNORE_POST'		=> ($row['foe']) ? sprintf($this->user->lang['POST_BY_FOE'], str_replace($this->web_root_path, $this->root_path, get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']))) : '',
			...
			$this->user_poster_data = $user_cache[$poster_id];
			...
			if (!empty($attachments[$row['post_id']]))
			{
				foreach ($attachments[$row['post_id']] as $attachment)
				{
					$this->template->assign_block_vars('postrow.attachment', array(
						'DISPLAY_ATTACHMENT'	=> str_replace($this->web_root_path, $this->root_path, $attachment))
					);
				}
			}
		...
		if ($all_marked_read)
		{
			...
			{
				$this->template->assign_vars(array(
					'U_VIEW_UNREAD_POST'	=> append_sid("{$this->root_path}viewtopic.$this->php_ext", "f=$forum_id&amp;t=$topic_id&amp;view=unread", true, false, true) . '#unread',
				));
			}
		}
